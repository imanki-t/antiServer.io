...
let wasKicked            = false;   // prevents double reconnect when kick triggers 'kicked' + 'end'
let stableConnectionTime = null;   // set after bot has been stable for MIN_STABLE_MS

// Bot must be connected this long before consecutiveFailures resets to 0.
// Prevents the 11-second rapid-reconnect loop caused by quick kicks.
const MIN_STABLE_MS = 120_000;  // 2 minutes

// Minimum floor for reconnect delay — never reconnect faster than this,
// regardless of consecutiveFailures value. Prevents server bans.
const MIN_RECONNECT_MS = 45_000; // 45 seconds

// ── Timer handles ──
let reconnectTimeout        = null;
let actionSchedulerTimeout  = null;
let activityIntervalHandle  = null;
let forceRejoinTimeoutHandle = null;
let spawnWatchdogTimeout    = null;  // kills hung connections that never fire 'spawn'
...
function clearAllTimers() {
  ...
  if (stableConnectionTime)     { clearTimeout(stableConnectionTime);     stableConnectionTime    = null; }
...
// NOTE: TCP probe removed — Aternos uses SRV DNS records so a raw TCP socket
// to the hostname:port always times out even when the server is online.
// Mineflayer resolves SRV records natively; let it handle connectivity.
...
async function startBot() {
  ...
  bot = mineflayer.createBot({
    ...
    version:  BOT_VERSION,
    auth:     'offline',
    // ── 1.21+ FIXES (GitHub issues #3775, #3776) ──────────────────────────
    // connectTimeout is unreliable on 1.21+ — use checkTimeoutInterval instead
    checkTimeoutInterval: 30000,   // ping check every 30s (replaces broken connectTimeout)
    closeTimeout:         240000,  // 4 min grace before force-close
    physicsEnabled:       false,   // CRITICAL: stops mineflayer sending physics packets
                                   // during the 1.21 config phase which causes silent kick
    hideErrors:           false,   // surface all internal mineflayer errors
    // ─────────────────────────────────────────────────────────────────────
  });

  // Log every connection stage so we know exactly where it hangs
  bot.once('login',  () => console.log('🔑 Bot logged in (pre-spawn)…'));
  bot.once('inject_allowed', () => console.log('💉 inject_allowed fired'));

  // ── WATCHDOG: if spawn hasn't fired in 60s after createBot, something is
  // silently hung (known mineflayer 1.21+ issue). Force-kill and retry. ──
  spawnWatchdogTimeout = setTimeout(() => {
    if (isBotOnline) return;
    console.warn('⚠️  Watchdog: no spawn in 60s — force-killing hung connection');
    if (bot) {
      bot.removeAllListeners();
  ...
  bot.once('spawn', () => {
    ...
    wasKicked      = false;
    // Don't reset consecutiveFailures immediately — wait until connection is stable
    // for MIN_STABLE_MS. This prevents the 11-second rapid-reconnect loop where
    // a quick kick resets failures to 0 then back to 1, giving a tiny 6s delay.
    stableConnectionTime = setTimeout(() => {
      consecutiveFailures = 0;
      console.log('✅ Connection stable — failure counter reset');
    }, MIN_STABLE_MS);
  ...
  bot.on('error', err => {
    ...
    // Always clean up and reconnect on any error — don't whitelist specific
    // error strings. Aternos restarts can produce many different error types
    // (ECONNREFUSED, ECONNRESET, socket hang up, connectTimeout, etc.) and
    // silently swallowing unknown ones leaves the bot dead with no reconnect.
    resetBotState();
    clearAllTimers();
    schedulerEpoch++;
    consecutiveFailures++;
  ...
  bot.on('end', async reason => {
    ...
    if (AUTO_REJOIN && !isShuttingDown && !wasKicked) scheduleReconnect();
  });
  ...
  bot.on('kicked', async reason => {
    ...
    schedulerEpoch++;
    consecutiveFailures++;
    wasKicked = true;  // tell 'end' handler not to schedule another reconnect

    if (AUTO_REJOIN && !isShuttingDown) {
      const isDuplicateLogin = rs.includes('duplicate_login');
      const isAfk = /idle|afk|terms/i.test(rs);
      // Always wait at least MIN_RECONNECT_MS after a kick — rapid reconnects after
      // kicks are the #1 reason bots get IP-banned on Aternos.
      const exponential = RECONNECT_DELAY * Math.pow(2, Math.min(consecutiveFailures, 6));
      const base  = Math.max(MIN_RECONNECT_MS, Math.min(exponential, 300_000));
      const delay = isDuplicateLogin ? 60_000
                  : isAfk           ? 90_000   // longer after AFK kick to avoid repeat ban
                  : base;
      if (isDuplicateLogin) console.log('⚠️  Duplicate login — waiting 60s for old session to expire');
...
function scheduleReconnect() {
  if (isShuttingDown) return;
  // Exponential backoff from base, but never below MIN_RECONNECT_MS.
  // Jitter (±20 %) makes reconnect timing look less robotic to the server.
  const exponential = RECONNECT_DELAY * Math.pow(2, Math.min(consecutiveFailures, 6));
  const floored     = Math.max(MIN_RECONNECT_MS, Math.min(exponential, 300_000));
  const jitter      = floored * 0.2 * (Math.random() - 0.5);
  const delay       = Math.floor(floored + jitter);
  console.log(`⏳ Reconnecting in ${(delay / 1000).toFixed(1)}s (attempt ${consecutiveFailures}, base ${(floored/1000).toFixed(0)}s)`);
  reconnectTimeout = setTimeout(startBot, delay);
}
...
function forceRejoinBot() {
  if (isShuttingDown) return;
  clearAllTimers();
  const delay = FIFTEEN_SECONDS;
  console.log(`⏳ Force-rejoining in ${delay / 1000}s (anti-AFK rejoin)`);
  forceRejoinTimeoutHandle = setTimeout(startBot, delay);
}
