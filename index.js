...
async function startBot() {
  ...
  bot = mineflayer.createBot({
    ...
    physicsEnabled: false,  // prevents internal errors on 1.21+ during handshake phase
  });

  // ── WATCHDOG: mineflayer's connectTimeout is unreliable — it often silently
  // hangs after a server restart with no error/end/spawn ever firing.
  // If spawn hasn't fired in 45s, force-kill and reschedule. ──
  spawnWatchdogTimeout = setTimeout(() => {
    if (isBotOnline) return; // already spawned, all good
    console.warn('⚠️  Spawn watchdog fired — bot stuck connecting, forcing restart');
    if (bot) {
      bot.removeAllListeners();
      try { bot._client?.end(); } catch { /* ignore */ }
      bot = null;
    }
    consecutiveFailures++;
    scheduleReconnect();
  ...
  bot.once('spawn', () => {
    // Cancel watchdog — we spawned successfully
    if (spawnWatchdogTimeout) { clearTimeout(spawnWatchdogTimeout); spawnWatchdogTimeout = null; }
    // Re-enable physics now that handshake is complete
